...
<div class="w-full bg-white rounded-2xl shadow-card border border-slate-100 overflow-hidden">
    ...
    <form action="{{ route('admin.testimonials.update',$testimonial) }}" method="POST" enctype="multipart/form-data" class="p-4 sm:p-6 lg:p-8 space-y-5">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
            ...
            <div><label class="block text-sm font-semibold mb-1.5">Foto</label><input type="file" name="photo" class="w-full rounded-xl border-slate-200 text-sm file:mr-3 file:py-2 file:px-3 file:rounded-lg file:border-0 file:bg-black file:text-white file:text-xs file:font-semibold hover:file:bg-black">@if($testimonial->photo)<img src="{{ asset('storage/'.$testimonial->photo) }}" class="mt-3 w-16 h-16 rounded-full object-cover border border-slate-200">@endif</div>
        </div>
        ...
        <div class="flex items-center gap-3"><label class="relative inline-flex items-center cursor-pointer"><input type="checkbox" name="is_active" value="1" @checked($testimonial->is_active) class="sr-only peer"><div class="w-11 h-6 bg-slate-200 rounded-full peer peer-checked:bg-black after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-full"></div></label><span class="text-sm font-medium">Aktif</span></div>
        <div class="flex flex-col-reverse sm:flex-row justify-end gap-3 pt-4 border-t border-slate-100"><a href="{{ route('admin.testimonials.index') }}" class="w-full sm:w-auto text-center px-5 py-2.5 rounded-xl border border-slate-200 text-sm font-medium hover:bg-slate-50">Batal</a><button class="w-full sm:w-auto px-6 py-2.5 rounded-xl bg-black text-white font-semibold text-sm hover:bg-black">Update</button></div>
    </form>
</div>
@endsection
