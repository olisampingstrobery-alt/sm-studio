    ...
    <div class="bg-white rounded-2xl shadow-card border border-slate-100 overflow-hidden">
        ...
        <form action="{{ route('admin.settings.update') }}" method="POST" class="p-6 space-y-6">
            ...
            <div class="grid md:grid-cols-2 gap-6">
                <div><label class="block text-sm font-semibold mb-1.5">Nama Website</label><input type="text" name="site_name" value="{{ ($settings->get('general') ?? collect())->where('key','site_name')->first()?->value ?? 'SM Studio' }}" class="w-full rounded-xl border-slate-200 focus:border-[#0F2A4A]"></div>
                <div><label class="block text-sm font-semibold mb-1.5">Tagline</label><input type="text" name="site_tagline" value="{{ ($settings->get('general') ?? collect())->where('key','site_tagline')->first()?->value ?? 'Unit Produksi PPLG SMK BPPI Baleendah — Dibangun siswa, siap bantu digital Anda' }}" class="w-full rounded-xl border-slate-200"></div>
                <div><label class="block text-sm font-semibold mb-1.5">Email</label><input type="email" name="contact_email" value="{{ ($settings->get('contact') ?? collect())->where('key','contact_email')->first()?->value ?? 'smstudiobppi@gmail.com' }}" placeholder="smstudiobppi@gmail.com" class="w-full rounded-xl border-slate-200"></div>
                <div><label class="block text-sm font-semibold mb-1.5">WhatsApp</label><input type="text" name="contact_whatsapp" value="{{ ($settings->get('contact') ?? collect())->where('key','contact_whatsapp')->first()?->value ?? '' }}" placeholder="+62..." class="w-full rounded-xl border-slate-200"></div>
