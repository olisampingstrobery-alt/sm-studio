    ...
    <div class="bg-white rounded-2xl shadow-card border border-slate-100 overflow-hidden">
        ...
        <form action="{{ route('admin.settings.update') }}" method="POST" class="p-6 space-y-6">
            ...
            <div class="grid md:grid-cols-2 gap-6">
                ...
                <div><label class="block text-sm font-semibold mb-1.5">Email</label><input type="email" name="contact_email" value="{{ $settings['contact']->where('key','contact_email')->first()?->value ?? '' }}" placeholder="hello@smstudio.id" class="w-full rounded-xl border-slate-200"></div>
