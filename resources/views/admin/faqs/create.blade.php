...
<div class="max-w-2xl bg-white rounded-2xl shadow-card border border-slate-100 overflow-hidden">
    ...
    <form action="{{ route('admin.faqs.store') }}" method="POST" class="p-6 space-y-5">
        ...
        <div class="flex justify-end gap-3 pt-4 border-t"><a href="{{ route('admin.faqs.index') }}" class="px-5 py-2.5 rounded-xl border">Batal</a><button class="px-6 py-2.5 rounded-xl bg-black text-white font-semibold">Simpan</button></div>
    </form>
</div>
@endsection
