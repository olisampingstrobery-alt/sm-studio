...
<section class="py-12 bg-gradient-to-b from-white to-[#F8FAFC]/30">
    <div class="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8">
        @if($portfolios->count() || true)
            {{-- React island — filter & search client-side tanpa reload (useDeferredValue + memo) --}}
            @php
                $portfolioPayload = ["portfolios" => $portfolios->getCollection()->map(fn($pf) => ["id"=>$pf->id,"title"=>$pf->title,"slug"=>$pf->slug,"category"=>["name"=>$pf->category->name ?? "Umum"],"client"=>["name"=>$pf->client->name ?? $pf->client_name ?? "Client"],"client_name"=>$pf->client_name ?? $pf->client->name ?? "Client","featured_image"=>$pf->featured_image,"is_featured"=>$pf->is_featured ?? false])->values()];
            @endphp
            ...
            <noscript>
                <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
                    ...
                    <a href="{{ route('portfolio.show', $pf->slug ?? $pf->id) }}" class="group bg-white rounded-[20px] border border-slate-100 overflow-hidden flex flex-col">
                        <div class="h-48 bg-slate-100 overflow-hidden">@if($pf->featured_image)<img src="{{ asset('storage/'.$pf->featured_image) }}" class="w-full h-full object-cover">@elseif($pf->images->count())<img src="{{ asset('storage/'.$pf->images->first()->image) }}" class="w-full h-full object-cover">@else<div class="w-full h-full grid place-items-center text-slate-400 text-sm">No Image</div>@endif</div>
                        <div class="p-5"><span class="text-xs font-semibold text-[#0F2A4A] bg-[#EFF6FF] px-2.5 py-1 rounded-full">{{ $pf->category->name ?? 'Umum' }}</span><h3 class="font-semibold text-[#0B1D33] mt-3">{{ $pf->title }}</h3></div>
                    </a>
                    @endforeach
                </div>
            </noscript>
